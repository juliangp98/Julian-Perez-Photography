...
A **needs-attention** strip at the top of `/admin/projects` surfaces stale inquiries (a `new-inquiry` untouched for 3+ days) and upcoming shoots (an event date within 30 days, still active), computed server-side from the full list. And editing a project — or quick-logging a status change — carries an opt-in **"email the client"** checkbox: when ticked it sends the client a branded `NotificationEmailTemplate` (a high-level "there's an update", with the new client-facing status label and a portal link — never the internal note) through the shared `sendClientUpdateEmail` helper in `src/lib/notify.ts`. The reciprocal "a client edited their details" email to Julian is on by default and muted with `NOTIFY_CLIENT_EDITS=false`. Every send is fire-and-forget via Resend (`level: "warning"` to Sentry) and no-ops when Resend is unconfigured.
