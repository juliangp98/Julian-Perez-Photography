...
- **[Sanity](https://sanity.io)** — embedded Studio at `/studio` powers the `/journal` section (round 13). Other content (services, portfolios, site settings) still lives in `src/lib/content.ts` until round 14 wires the dormant schemas
...
| `NEXT_PUBLIC_SANITY_PROJECT_ID` | no | Sanity project ID (from Project → API). Without it, `/journal` shows a "coming soon" placeholder and `/studio` boots Sanity's project-connect screen |
...
- **Editing**: go to `/studio`, sign in with the Sanity account that owns the project, and publish a `Journal Post`. New posts appear on `/journal` within ~60 seconds (driven by `next: { revalidate: 60 }` on the GROQ fetches).
- **Schema**: `sanity/schemas/journalPost.ts` — title, slug, publishedAt, excerpt, cover image, portable-text body (with inline images), tags, and a `featured` boolean reserved for future home-page surfacing.
- **No env vars set?** `/journal` renders a "Coming soon" placeholder and `/studio` boots Sanity's project-connect screen. Neither breaks the build or the rest of the site.
- **Other content** (services, portfolios, site settings) still lives in `src/lib/content.ts`. Those schemas are written in `sanity/schemas/` but deliberately not registered in `sanity.config.ts` yet — see `sanity/README.md` for the round-14 migration plan.
